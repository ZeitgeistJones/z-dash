...
// GeckoTerminal: batch fetch token data, 30 addresses per call, no API key needed
async function fetchGeckoTerminalPrices(addresses) {
  const lookup = {};
  const batchSize = 30;
  for (let i = 0; i < addresses.length; i += batchSize) {
    ...
    try {
      const res = await fetch(
        `https://api.geckoterminal.com/api/v2/networks/base/tokens/multi/${joined}`,
        {
      ...
      if (json.data && Array.isArray(json.data)) {
        for (const token of json.data) {
          ...
          if (!addr) continue;
          lookup[addr] = {
            priceUsd: token.attributes?.price_usd ? parseFloat(token.attributes.price_usd) : null,
            marketCapUsd: token.attributes?.market_cap_usd ? parseFloat(token.attributes.market_cap_usd) : null,
            fdvUsd: token.attributes?.fdv_usd ? parseFloat(token.attributes.fdv_usd) : null,
            volume24h: token.attributes?.volume_usd?.h24 ? parseFloat(token.attributes.volume_usd.h24) : null,
            priceChange24h: token.attributes?.price_change_percentage?.h24 ? parseFloat(token.attributes.price_change_percentage.h24) : null,
          };
        }
      }
    } catch {
    ...
    // Respect rate limit: ~30 calls/min, small delay between batches
...
export async function getDashboardData() {
  ...
  // Step 2: Batch-fetch ALL prices from GeckoTerminal (6 API calls for 178 tokens)
  const allAddresses = tokens.filter(t => t.address).map(t => t.address);
  const priceLookup = await fetchGeckoTerminalPrices(allAddresses);
  ...
  const enrichedWithAddress = tokensWithAddress.map((token) => {
    ...
    const gt = priceLookup[addrKey] || null;

    // Use market_cap if available, fall back to FDV
    const marketCap = gt?.marketCapUsd || gt?.fdvUsd || null;
    const priceChange = gt?.priceChange24h ?? null;
    ...
    return {
      ...
      signal,
      signalScore:   gt ? getSignalScore(duneRow?.["Vol Grw %"] ?? null, priceChange) : null,
      read,
    };
  });
